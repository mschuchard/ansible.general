...
def test_packer_build_except_onerror_force(capfd):
    ...
    utils.set_module_args({'config_dir': '/tmp', 'force': True, 'on_error': 'abort', 'only': ['null.this', 'null.that']})
    with pytest.raises(SystemExit, match='1'):
        packer_build.main()
...
def test_packer_build_except_parallel_timestamp(capfd):
    ...
    utils.set_module_args({'config_dir': '/tmp', 'excepts': ['null.this', 'null.that'], 'parallel_builds': 1, 'timestamp_ui': True})
    with pytest.raises(SystemExit, match='1'):
        packer_build.main()
