...
def test_faas_build_no_cache_globals(capfd):
    ...
    utils.set_module_args({'config_file': f'{utils.fixtures_dir()}/stack.yaml', 'cache': False, 'filter': '*gif*', 'regex': 'fn[0-9]_.*'})
    with pytest.raises(SystemExit, match='1'):
        faas_build.main()
...
def test_faas_build_stack_pull_shrinkwrap(capfd):
    ...
    utils.set_module_args({'config_file': f'{utils.fixtures_dir()}/stack.yaml', 'stack_pull': False, 'env_subst': False, 'pull': True, 'shrinkwrap': True})
    with pytest.raises(SystemExit, match='1'):
        faas_build.main()
...
def test_faas_build_build_option_squash(capfd):
    ...
    utils.set_module_args({'config_file': f'{utils.fixtures_dir()}/stack.yaml', 'build_option': ['dev', 'verbose'], 'squash': True})
    with pytest.raises(SystemExit, match='1'):
        faas_build.main()
...
def test_faas_build_direct_params(capfd):
    ...
    utils.set_module_args({'image': 'my_image', 'lang': 'python', 'handler': f'{utils.fixtures_dir()}', 'name': 'my_fn', 'squash': True})
    with pytest.raises(SystemExit, match='1'):
        faas_build.main()
...
def test_faas_build_parallel_copy_extra(capfd):
    ...
    utils.set_module_args(
        {
            'config_file': f'{utils.fixtures_dir()}/stack.yaml',
            'parallel': 4,
            'copy_extra': [f'{utils.fixtures_dir()}/foo.pkrvars.hcl', f'{utils.fixtures_dir()}/goss.yaml'],
        }
    )
    with pytest.raises(SystemExit, match='1'):
        faas_build.main()
    ...
    assert f'{utils.fixtures_dir()}/foo.pkrvars.hcl' in info['cmd']
    assert f'{utils.fixtures_dir()}/goss.yaml' in info['cmd']
    assert f'{utils.fixtures_dir()}/stack.yaml' in info['cmd']
    assert '[\'openfaas\'] is the only valid "provider.name" for the OpenFaaS CLI, but you gave: \n' == info['stdout']
    ...
    utils.set_module_args(
        {
            ...
            'build_option': ['dev'],
            'copy_extra': [f'{utils.fixtures_dir()}/foo.pkrvars.hcl'],
            'parallel': 2,
            'squash': True,
            'tag': 'branch',
        }
